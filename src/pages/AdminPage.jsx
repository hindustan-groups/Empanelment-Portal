...
export default function AdminPage({ isAuthenticated, onLogout }) {
  ...
  const fetchVendors = async () => {
    ...
    // Import seed vendors from dedicated data file (not inline)
    const { SEED_VENDORS: seedApps } = await import('../data/seedVendors.js');
  ...
  const handleChangePassword = (e) => {
    e.preventDefault();
    const current = localStorage.getItem('hipro_admin_pwd') || import.meta.env.VITE_ADMIN_PASSWORD || 'HindustanAdmin2026#';
    if (adminPassword !== current) {
