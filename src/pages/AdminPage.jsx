...
export default function AdminPage({ isAuthenticated, onLogout }) {
  ...
  const handleUpdateStatus = async (trackingId, newStatus, stage, remark) => {
    ...
    setVendors(prev => prev.map(v => v.tracking_id === trackingId
      ? { ...v, status: newStatus, current_stage: stage, admin_remarks: remark ?? v.admin_remarks }
      : v
    ));
    if (selectedVendor?.tracking_id === trackingId) {
      setSelectedVendor(prev => ({ ...prev, status: newStatus, current_stage: stage, admin_remarks: remark ?? prev.admin_remarks }));
    }
  };
