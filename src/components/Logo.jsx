import React from 'react';

export default function Logo({ className = '', height = 40, showText = true, darkBg = false }) {
  return (
    <div
      className={`select-none ${className}`}
      style={{
        display: 'inline-flex',
        alignItems: 'center',
        gap: '0.65rem',
        backgroundColor: '#FFFFFF',
        padding: '0.35rem 0.85rem',
        borderRadius: 12,
        border: '1.5px solid #E2E8F0',
        boxShadow: '0 4px 12px rgba(0,0,0,0.08)'
      }}
    >
      <img
        src="/hipro-logo.png"
        alt="HiPRO Hindustan Projects Logo"
        style={{ height: height, width: 'auto', objectFit: 'contain', mixBlendMode: 'normal', display: 'block' }}
      />

      {showText && (
        <div style={{ display: 'flex', flexDirection: 'column', borderLeft: '1.5px solid #CBD5E1', paddingLeft: '0.65rem' }}>
          <span style={{ fontSize: `${Math.max(12, height * 0.35)}px`, fontWeight: 900, color: '#0F172A', letterSpacing: '0.14em', textTransform: 'uppercase', lineHeight: 1.1 }}>
            HINDUSTAN
          </span>
          <span style={{ fontSize: `${Math.max(10, height * 0.28)}px`, fontWeight: 800, color: '#ED1C24', letterSpacing: '0.18em', textTransform: 'uppercase', lineHeight: 1.1 }}>
            PROJECTS
          </span>
        </div>
      )}
    </div>
  );
}
